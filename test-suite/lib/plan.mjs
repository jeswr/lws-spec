...
export function classifyStatement(statement, caseResults) {
  ...
  // sibling vector blocks the pass verdict (a FAIL above still dominates —
  // filtering must not mask a real failure). The filtered category itself
  // ranks BELOW the specific non-execution diagnostics (setup error,
  // unrealizable, precondition), so an actionable reason is never hidden
  // behind "partial run".
  if (pick('pass') && !pick('skip-filtered')) {
    return { category: 'pass', detail: 'all executed wired vectors passed', cases };
  }
  if (pick('error-setup')) {
