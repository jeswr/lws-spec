...
(conjunction); a grant carrying `odrl:prohibition`/`odrl:obligation` rules derives nothing
(their decision-time composition is not defined by this profile version).
...
operator, or context key is an **encode error** (fail loud, never silently dropped).
`xsd:dateTime` values are presented as their canonical RFC 3339 UTC (`Z`) lexical forms,
under which chronological and lexicographic order coincide.
